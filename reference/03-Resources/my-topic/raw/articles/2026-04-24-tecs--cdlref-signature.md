...
date: 2026-04-24
source-type: article
source-url: E:\Crepo\github-toppers\tecs-docs\docs\_build\html\tecs\CDLref_signature.html
title: シグニチャ記述
compiled: true
tags: [tecs, toppers, documentation]
description: "TECS ドキュメント: シグニチャ記述"
---
...
```
signaturesInputOutput{ERsetOutput([in]int8_tval);// in 基本指定子ERgetInput([out]int8_t*val);// out 基本指定子ERnoParameter(void);// 引数なし};
```
...
```
[context("task")]signaturesSignature{};
```
...
```
signaturesInOut{ERin([in,string]char*buf);/* in では、呼び元が文字列を用意する */ERout([out,string(64)]char*buf);/* out では、呼び元が長さ 64 のバッファを用意する */ERinout([inout,string(*len)]char*buf,[inout]uint16_t*len);/* inout では、呼び元が長さ len のバッファを用意する *//* *len の出力方向は文字列長さを返す */};
```
...
```
[deviate]signaturesOutStringPointor{ERgetErrorMessage([out]constchar_t**message);};
```
...
```
signaturesSendReceive{ERsendMessage([send(sAlloc),size_is(len)]int8_t*buf,[in]int16_tlen);ERreceiveMessage([receive(sAlloc),size_is(len)]int8_t**buf,[out]int16_tlen);};
```
...
```
signaturesInputOutput{ERputMessage([in,size_is(len)]constint8_t*buf,[in]int16_tlen);ERgetMessage([out,size_is(*len),count_is(*len)]int8_t*buf,[inout]int16_t*len);};
```
...
```
[deviate]signaturesAlloc{ERalloc([in]size_tlen,[out]void**p);ERdealloc([in]void*p);};
```
